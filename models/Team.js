(function() {
  ...
  teamSchema.statics.createMW = (req, res, next) => {
    if (!req.body.team || !req.body.leagueId) {
      return res
        .status(400)
        .send('Both Team name & League ID are required to create a new Team');
    }
    ...
    mongoose
      .model('Team')
      .findOne(
        { name: titleReg, league: req.body.leagueId },
        (err, foundTeam) => {
          if (err) {
          ...
          if (foundTeam) {
            return res
              .status(400)
              .send(
                'A Team with this name already exists in this League – Please try again with a different Team name'
              );
          }
          mongoose
            .model('League')
            .findById(req.body.leagueId, (err, foundLeague) => {
              if (err) {
                return res.status(400).send(err);
              }
              if (!foundLeague) {
                return res
                  .status(400)
                  .send(
                    'League not found – Please double check that the League ID is correct'
                  );
              }
              mongoose
                .model('Team')
                .findOne(
                  { owner: req.user, league: foundLeague.id },
                  (err, foundTeam) => {
                    if (err) {
                      return res.status(400).send(err);
                    }
                    ...
                    mongoose
                      ...
                      .then(foundUser => {
                        ...
                        newTeam.imgUrl =
                          'https://s3-us-west-1.amazonaws.com/groupffl/default_image.png';
  ...
  teamSchema.statics.getMyTeam = (req, res) => {
    mongoose
      .model('Team')
      .findOne(
        { owner: req.user, league: req.params.leagueId },
        (err, myTeam) => {
          if (err) {
            return res.status(400).send(err);
          }
      ...
      mongoose
        ...
        .findOne(
          ...
          (err, myTeam) => {
            ...
            myTeam.imgUrl =
              process.env.AWS_URL + '/' + process.env.AWS_BUCKET + '/' + key;
